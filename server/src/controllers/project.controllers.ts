...
export const updateChecklistItem = async (
  ...
  try {
    ...
    const { id: projectId, itemId } = req.params as {
      id: string;
      itemId: string;
    };

    const updated = await ProjectService.updateChecklistItem(
      projectId, // Servis artık log için bunu bekliyor
      itemId,
      req.userId!,
      req.body,
    );
...
export const deleteChecklistItem = async (
  ...
  try {
    ...
    await ProjectService.deleteChecklistItem(
      projectId, // Servis artık log için bunu bekliyor
      itemId,
      req.userId!,
    );
...
export const addResource = async (
  ...
  try {
    ...
    const resource = await ProjectService.addResource(
      projectId,
      userId,
      resourceData,
    );
...
export const deleteResource = async (
  ...
  try {
    ...
    const { id: projectId, resourceId } = req.params as {
      id: string;
      resourceId: string;
    };

    await ProjectService.deleteResource(
      projectId, // Servis artık log için bunu bekliyor
      resourceId,
      req.userId!,
    );
...
export const addMember = async (
  ...
  try {
    ...
    if (!ownerId) return next(new AppError("User ID not found", 400));
    if (!email || !role) {
...
export const removeMember = async (
  ...
  try {
    ...
    const { id: projectId, userId: memberIdToRemove } = req.params as {
      id: string;
      userId: string;
    };
    const ownerId = req.userId;
...
export const inviteMember = async (
  ...
  try {
    ...
    const result = await ProjectService.sendInvitation(
      projectId,
      ownerId,
      email,
      role,
    ...
    if (result.userExists && result.userId) {
      ...
      const inviteToken = jwt.sign(
        { email, projectId, role, type: "PROJECT_INVITE" },
...
export const completeExternalInvitation = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
