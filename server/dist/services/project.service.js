...
export const ProjectService = {
    async createActivity(projectId, userId, action, entityName, entityId) {
        try {
    ...
    async createProject(data, userId) {
        ...
        await this.createActivity(project.id, userId, "PROJECT_CREATED", project.name);
    ...
    async updateProject(projectId, userId, data) {
        ...
        // 3. Eğer tip PAID ise ve yeni bir fiyat gelmemişse mevcut fiyatı kontrol et
            ...
            const currentPrice = data.price !== undefined ? Number(data.price) : Number(project.price);
            if (currentPrice <= 0) {
                throw new AppError("Paid projeler için 0'dan büyük bir fiyat girmelisiniz.", 400);
            }
    ...
    // Checklist CRUD
    async addChecklistItem(projectId, userId, title) {
        ...
        await this.createActivity(projectId, userId, "CHECKLIST_ADDED", item.title, item.id);
        return item;
    },
    async updateChecklistItem(projectId, // <-- Log için bunu ekledik
    itemId, userId, data) {
        ...
        if (data.isDone !== undefined) {
            ...
            await this.createActivity(projectId, userId, action, updated.title, itemId);
        }
        else {
            // Sadece başlık veya aciliyet değiştiyse
            await this.createActivity(projectId, userId, "CHECKLIST_UPDATED", updated.title, itemId);
        }
        return updated;
    },
    async deleteChecklistItem(projectId, itemId, userId) {
        const item = await prisma.checklistItem.findUnique({
            where: { id: itemId },
        });
        if (!item)
        ...
        await this.createActivity(projectId, userId, "CHECKLIST_DELETED", item.title);
        return await prisma.checklistItem.delete({ where: { id: itemId } });
    },
    async addResource(projectId, userId, data) {
        await this.getProjectById(projectId, userId);
        ...
        await this.createActivity(projectId, userId, "RESOURCE_ADDED", resource.title, resource.id);
        return resource;
    },
    async deleteResource(projectId, resourceId, userId) {
        ...
        if (resource.project.ownerId !== userId) {
            throw new AppError("Unauthorized: Only the project owner can delete resources", 403);
        }
        ...
        await this.createActivity(projectId, userId, "RESOURCE_DELETED", resource.title);
        // 2. ADIM: Veritabanından sil
        return await prisma.projectResource.delete({
            where: { id: resourceId },
        });
    },
    async addMember(projectId, ownerId, email, projectRole) {
        ...
        if (userToAdd && userToAdd.id === ownerId) {
            throw new AppError("You are already the owner of this project. You cannot add yourself as a member.", 400);
        }
        if (!userToAdd) {
    ...
    async removeMember(projectId, ownerId, userIdToRemove) {
        const project = await prisma.project.findUnique({
            where: { id: projectId },
        });
        if (!project) {
    ...
    async sendInvitation(projectId, invitedById, email, role) {
